...
数据库: PeeweeSqliteDatabase = PeeweeSqliteDatabase(
    数据库路径,
    pragmas={
        "journal_mode": "wal",      # 开启 WAL 模式
        "busy_timeout": 10000,      # 发生锁时等待10秒，而不是立即报错
        "cache_size": -64000,       # 设置缓存大小为 64000 KB（约 64 MB）
        "foreign_keys": 1,          # 启用外键约束
        "synchronous": 1            # 设置同步模式为 NORMAL
    }
)
...
class 类_SQLite数据库(类_基础数据库):
    ...
    def __init__(self) -> None:
        ...
        self.数据库.connect(reuse_if_open=True)
    ...
    def 加载K线数据(
        ...
        """读取K线数据"""
        with self.数据库.connection_context():
            查询语句: ModelSelect = (
